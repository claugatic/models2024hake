...
baseplot<-function(fit,name, trans=function(x)x, makeroom=NA, drop=1, ci=TRUE, line=TRUE, ...){
...
addlines<-function(fit,name, trans=function(x)x, with.dot=FALSE, drop=1, ...){
  idx<-1:(nrow(fit[[name]])-drop)
  est<-trans(fit[[name]][idx,1])
  low<-trans(fit[[name]][idx,3])
  hig<-trans(fit[[name]][idx,4])
...
xtab<-function(x,caption='Table X.', file=stdout(), width='"100%"', cornername='', dec=rep(1,ncol(x)),big.mark=","){
  nc<-ncol(x)
  ...
  x<-sapply(1:ncol(x),function(i)sub('NA','  ',formatC(big.mark=big.mark,round(x[,i],dec[i]),digits=dec[i], format='f')))
  for(i in 1:nrow(x)){
