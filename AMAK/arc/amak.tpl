...
//  Index values
    ...
    switch (ind_sel_opt(k))
    {
      case 1 : // Selectivity coefficients  indices
      {
       ...
       // Number of selectivity changes is equal to the number of vectors (yr 1 is baseline)
        ...
        {
          for (int j=nselages_in_ind(k)+1;j<=nages;j++) 
          {
 ...
 LOCAL_CALCS
  ...
  write_input_log << "# Multinomial Offset indices: "<<endl;
  write_input_log<<offset_ind<<endl;
  write_input_log << "# Multinomial Offset fisheries: "<<endl;
  write_input_log<<offset_fsh<<endl;
...
FUNCTION Get_Selectivity
  ...
  for (k=1;k<=nfsh;k++)
  {
    ...
    {
       int isel_ch_tmp = 1 ;
       dvar_matrix tempsel(1,n_sel_ch_fsh(k),1,nselages_fsh(k));
       ...
       tempsel = a(agetmp);
       int j=1;
       // cout <<tempsel<<endl;exit(1);
       log_sel_fsh(k,styr)(1,nselages_fsh(k)) = tempsel(j);;
       avgsel_fsh(k,j)  = mean(log_sel_spl_fsh(k,j)); //log(mean(mfexp(log_sel_fsh(k,styr))));
       log_sel_fsh(k,styr)(nselages_fsh(k),nages) = log_sel_fsh(k,styr,nselages_fsh(k))  ; 
...
FUNCTION Get_Mortality
  ...
  if (npars_rw_M>0)
  {
    ...
    for (i=styr+1;i<=endyr;i++)
    {
      ...
      if (i==yrs_rw_M(ii))
      {
        natmort(i) = natmort(i-1)*mfexp(M_rw(ii));
        ii++;
        if (npars_rw_M < ii) ii=npars_rw_M;
      }
      else
        natmort(i) = natmort(i-1);
    }
  }
  for (i=styr;i<=endyr;i++)
    Z(i) = natmort(i);
...
FUNCTION Compute_priors
  ...
  for (k=1;k<=nind;k++)
  {
    ...
    if (active(log_rw_q_ind(k)))
      ...
      {
        post_priors_indq(k) += square(log_rw_q_ind(k,i))/ (2.*sigma_rw_q(k,yrs_rw_q(k,i))*sigma_rw_q(k,yrs_rw_q(k,i))) ;
  ...
  if (active(M))
    post_priors(1) += square(log(M/natmortprior))/(2*cvnatmortprior*cvnatmortprior); 
  if (active(M_rw))
  ...
  for (int iscen=1;iscen<=5;iscen++)
  {
   // Future Sp_Biom set equal to estimated Sp_Biom w/ right lag
    ...
    for (i=styr_fut; i<=endyr_fut; i++)
    {
      ...
      future_biomass(iscen,i) = Sp_Biom_future(i);
      // cout<<iscen<<" "<<i<<" "<< future_biomass(iscen,i) <<" "<<nage_future(i)(1,4)<<endl;
    }
    ...
    for (k=1;k<=nind;k++)
    {
      ...
      // {
        if (ii<=yrs_ind(k).indexmax())
        {
          if (yrs_ind(k,ii)==i)
          {
            report << i<< " "<< obs_ind(k,ii) << " "<< pred_ind(k,ii) <<endl;
            ii++;
          }
          //else
